...
Enllaç al repositori GIT: https://github.com/albertmoyanadal/practica_2_ad.git
...
data_medicacion %>% 
  ...
  geom_violin(aes(color = medicacion), width = 0.3, alpha = 0.2,trim=FALSE) +
  geom_boxplot(aes(color = medicacion), width = 0.1, alpha = 0.2)+
  geom_jitter(aes(color = medicacion), alpha = 0.5, 
              show.legend = TRUE, 
              position = position_jitter(width = 0.3, seed = 0)) + 
  scale_x_discrete(labels = c("No", "Sí")) +
  labs(x = "Consum de Medicació", y = "Índex de Qualitat Vocal (VHI)") +
  theme_minimal(base_size = 14) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))+scale_color_manual(values = c("#E69F00","#56B4E9"))
```
