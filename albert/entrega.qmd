...
```
Voleu ficar els QQPlots??? 

ExplicaciÓ:

Sigui $n$ la longitud de la mostra, llavors per $ k = 1, \ldots n$ hi ha un $k/n$-cantil

Llavors la recta blava mos representa la recta cuantil-cuantil i la franja blava on haurien d'estar en 95% de les dades que com podem observar es surten
```{r}
muestra = si_medicacion$VHI
  qqPlot((muestra-mean(muestra))/sqrt(var(muestra)), distribution="norm", mean = 0, sd = 1, id=FALSE , grid = FALSE,col = carPalette()[1], col.lines = carPalette()[8], main = 'Relación cuantiles con una normal')
...
qqPlot(muestra2, distribution="norm", mean = mean(muestra2), sd = sqrt(var(muestra2)), id=FALSE)
```
